...
data class WalkingUiState(val method: String = "全身行走", val running: Boolean = false, val elapsed: Int = 0, val note: String = "", val saving: Boolean = false, val message: String? = null, val history: List<WalkingSessionEntity> = emptyList())
...
class WalkingViewModel @Inject constructor(
    ...
    fun start() {
        ...
        _state.value = _state.value.copy(running = true, elapsed = 0, message = null)
        persist(); startTicker()
    }
    fun finish() {
        if (!_state.value.running) return
    ...
    fun save() {
        val s = _state.value
        if (s.running || s.elapsed <= 0 || s.saving) return
        viewModelScope.launch {
            _state.value = s.copy(saving = true, message = null)
            runCatching { repo.saveWalking(WalkingSessionEntity(date = now(), durationSeconds = s.elapsed, method = s.method, observation = s.note.trim())) }
                .onSuccess { clearPersistence(); _state.value = s.copy(running = false, elapsed = 0, note = "", saving = false, message = "已保存经行"); refresh() }
    ...
    private fun persist() { val s = _state.value; if (s.elapsed <= 0 && !s.running && s.note.isBlank()) return; prefs.edit().putBoolean("active", s.elapsed > 0 || s.running || s.note.isNotBlank()).putString("method", s.method).putInt("elapsed", pausedElapsed).putLong("started_at", startedAtMillis ?: 0L).putString("note", s.note).apply() }
    private fun clearPersistence() { prefs.edit().clear().apply() }
    private fun refresh() { viewModelScope.launch { runCatching { repo.getRecentWalking() }.onSuccess { _state.value = _state.value.copy(history = it) } } }
