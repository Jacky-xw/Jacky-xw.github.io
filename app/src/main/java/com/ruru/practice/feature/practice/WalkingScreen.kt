            ...
            item {
                Card {
                    Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        Button(onClick = viewModel::save, enabled = state.elapsed > 0 && !state.saving && !state.saved, modifier = Modifier.fillMaxWidth()) {
                            Text(when { state.saving -> "保存中…"; state.saved -> "已保存"; else -> "保存经行" })
                        }
