    ...
    Surface(modifier = modifier.fillMaxSize()) {
        LazyColumn(
            ...
            if (state.phase == MeditationPhase.COMPLETED || state.phase == MeditationPhase.SAVED) {
                item {
                    CompletionCard(state, viewModel)
                }
            }
        ...
        Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (state.phase == MeditationPhase.COMPLETED) {
                Button(
                    ...
                    Text(if (state.saveState.isSaving) "保存中…" else "保存练习")
                }
            }
...
@Composable
private fun HistoryCard(session: MeditationSessionEntity) {
    Card(modifier = Modifier.fillMaxWidth()) {
...
private fun phaseLabel(phase: MeditationPhase): String = when (phase) {
    ...
    MeditationPhase.SAVED -> "已保存到练习历史"
}
