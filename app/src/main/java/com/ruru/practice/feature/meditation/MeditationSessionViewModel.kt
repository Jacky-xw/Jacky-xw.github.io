...
class MeditationSessionViewModel @Inject constructor(
    ...
    fun save() {
        ...
        viewModelScope.launch {
            ...
            runCatchingSuspend {
                saveMeditation(MeditationSessionEntity(date = nowLabel(), durationSeconds = current.elapsedSeconds, observation = current.observation.trim(), afterState = current.afterState.trim(), practiceStep = current.practiceStep, stepTitle = AnapanStepCatalog.title(current.practiceStep)))
            }.onSuccess {
                clearPersistence()
