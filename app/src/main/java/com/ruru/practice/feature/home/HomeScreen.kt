            ...
            item {
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        FeedbackRow("八戒自检", state.todayEightPreceptCount > 0, "${state.todayEightPreceptCount} 次")
...
@Composable
private fun SessionSummaryCard(session: MeditationSessionEntity) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(session.date, style = MaterialTheme.typography.labelLarge)
                Text(formatDuration(session.durationSeconds), fontWeight = FontWeight.SemiBold)
            }
            if (session.observation.isNotBlank()) {
                Text(session.observation, maxLines = 2, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
