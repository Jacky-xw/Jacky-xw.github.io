...
fun ObservationScreen(viewModel: ObservationViewModel = hiltViewModel()) {
    ...
    var selectedIds by remember(state.tab) { mutableStateOf(setOf<Long>()) }
        ...
        LazyColumn(
            ...
            if (state.tab == 0) {
                if (state.recentEvents.isEmpty()) item { Text("还没有当下观察。", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                    ...
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Row(Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Checkbox(checked = event.id in selectedIds, onCheckedChange = { checked -> selectedIds = if (checked) selectedIds + event.id else selectedIds - event.id })
                ...
                if (state.recentAggregates.isEmpty()) item { Text("还没有五蕴拆解。", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                    ...
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Row(Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Checkbox(checked = aggregate.id in selectedIds, onCheckedChange = { checked -> selectedIds = if (checked) selectedIds + aggregate.id else selectedIds - aggregate.id })
                            Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                ...
                if (state.recentDependentOriginations.isEmpty()) item { Text("还没有缘起链条记录。", color = MaterialTheme.colorScheme.onSurfaceVariant) }
                    ...
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Row(Modifier.fillMaxWidth().padding(12.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Checkbox(checked = chain.id in selectedIds, onCheckedChange = { checked -> selectedIds = if (checked) selectedIds + chain.id else selectedIds - chain.id })
                            Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(5.dp)) {
